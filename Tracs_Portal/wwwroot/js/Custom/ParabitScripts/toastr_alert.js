...
function changeToTapToDismissWithClose(timeout, extended_timeout) {
    timeout = timeout || 3000;
    extended_timeout = extended_timeout || 8000;
    toastr.options = {
        "tapToDismiss": false,
...
function changeFeedbackMessage(alertType, message, title, additionalInfo, strictDialog, showCloseButton) {
    ...
    if (message.toLowerCase().indexOf('log in again') > -1)
        additionalInfo = '<br /><br /><br /><button onclick = "href" type="button" class="para-btn clear">Login</button>'.replace('href', 'location.href = \'' + "/Login/Home/Logoff" + '\'')
    ...
    if (alertType == 'none') {
        $('div.toast-none').css("padding-left", "0px");
        $('div.toast-none').css("padding-right", "0px");
        $('div.toast-none').css("padding-top", "0px");
        $('div.toast-none').css("width", "30%");
        $('.toast-close-button').css('position', 'static');
        $('.toast-close-button').css('padding-right', '5px');
